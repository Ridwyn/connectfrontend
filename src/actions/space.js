...
export const customStatuses = async(payload) =>{
    let res = await    axios.post(`${server}/api/space/custom_statuses`,payload,{headers:{authorization:localStorage.getItem('token').split('"')[1]}})
    return res.data
};
